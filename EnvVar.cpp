      ...
      scope_(es_invalid)
{
    ...
    RegOpenKeyExA(key, subKeyName, 0, KEY_QUERY_VALUE | KEY_SET_VALUE, &subKey);
    ...
    if (ERROR_SUCCESS != status) {
        ...
        // EnvVar object.
        data = new BYTE [size];
        RegQueryValueExA(subKey, name_.c_str(), 0, NULL, data, &size);
...
void EnvVar::set (std::string const &text)
{
    ...
    // Write the new value to the registry.
    RegOpenKeyExA(key, subKeyName, 0, KEY_SET_VALUE, &subKey);
    RegSetValueExA(subKey,
                  name_.c_str(),
                  0,
                  REG_EXPAND_SZ,
...
void EnvVar::unset ()
{
    ...
    RegOpenKeyExA(key, subKeyName, 0, KEY_SET_VALUE, &subKey);
    RegDeleteValueA(subKey, name_.c_str());
    RegCloseKey(key);
