...
//this is a modified EDITENV, which simply adds a function at the end which makes it do the same thing as setPath, but from custom actions
//so it basically retrives the CustomArgument field, translate it to a char, and pass it into pathAdd with the es_system flag
#include <limits>
...
void pathAdd(env_scope scope, char const *path) {
  ...
  while (sizeMax != pos) {
    if (((0 == pos) || (';' == value[pos - 1])) &&
        ((pos + length == value.length()) || (';' == value[pos + length]))) {
...
unsigned int setPath(unsigned long hInstall) {
  wchar_t vbuff[500] = {0};
  unsigned long vlen = 500;
  unsigned int gp =
  MsiGetPropertyW(hInstall, L"CustomActionData", vbuff, &vlen);

  if (gp == ERROR_SUCCESS) {
    char str[500]; // Allocate memory for the converted string
    size_t convertedChars = 0;
    wcstombs_s(&convertedChars, str, sizeof(str), vbuff, _TRUNCATE);
    pathAdd(editenv::es_system, str);
  }
  return 0;
}
